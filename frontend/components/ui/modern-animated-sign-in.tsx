...
import { cn } from '../../lib/utils'; // Adjusted path for Vite project structure
...
const AnimatedForm = memo(function AnimatedForm({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      // Just call onSubmit, let parent handle validation outcome or redirects
      onSubmit(event);
...
interface AuthTabsProps {
  ...
  // Added optional prop to support injecting left side content if needed, 
  // though standard implementation handles it below in Demo or similar wrapper.
  leftChild?: ReactNode;
}
...
const AuthTabs = memo(function AuthTabs({
  ...
  return (
    <div className='flex max-lg:justify-center w-full md:w-auto min-h-screen'>
      {/* Left Side: If passed (for custom injection) or just empty if parent handles it differently, 
         but based on prompt Demo usage, the layout is handled by the parent 'Demo' component 
         which splits the screen. 
         However, the user's AuthScreen might want to use AuthTabs directly.
         Let's keep it simple and clean as per user code. */}

      {/* Right Side */}
      <div className='w-full h-full flex flex-col justify-center items-center max-lg:px-[10%] bg-white dark:bg-zinc-950'>
        <AnimatedForm
          {...formFields}
          fieldPerRow={1}
